...
from .db import init as db_init, create_position, add_fill, update_peak_and_trail, close_position
from .config import CORE_STOP_PCT, SCOUT_STOP_PCT, LOG_JSON_PATH, LOG_TEXT_PATH
...
class TradeEngine:
    ...
    def check_exits(self, token: str, price: float) -> bool:
        ...
        if price <= (1.0 - stop_pct / 100.0) * max(1e-9, peak):
            fill = self.broker.market_sell(token, 0.0)  # placeholder qty in dry-run
			close_position(pid)
		...
		if peak and price <= (1.0 - trail / 100.0) * peak:
            fill = self.broker.market_sell(token, 0.0)
			close_position(pid)
            self.live.pop(token, None)
